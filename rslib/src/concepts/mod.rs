...
/// Weakness for a concept given the mean recall of its cards that have a memory
/// state. With no such cards, weakness is the maximum, 1.0.
fn weakness(recall_sum: f64, recall_count: u32) -> f64 {
    if recall_count == 0 {
        1.0
    } else {
        (1.0 - recall_sum / recall_count as f64).clamp(0.0, 1.0)
    }
}

/// Compute per-concept mastery + NTR over the given cards. Set-based: one pass
/// over the cards, no per-card queries. Concepts are returned in taxonomy
/// order so output is deterministic. Concepts with no cards in scope are still
/// reported (cards_total 0, ntr = topic_weight).
pub fn compute_mastery(taxonomy: &Taxonomy, cards: &[CardConcepts]) -> Vec<ConceptMastery> {
    ...
    taxonomy
        ...
        .map(|rule| {
            ...
            let ntr = rule.weight() * weakness(a.recall_sum, a.recall_count);
            ConceptMastery {
...
pub fn order_by_ntr(taxonomy: &Taxonomy, cards: &[CardConcepts]) -> Vec<QueueEntry> {
...
mod test {
    ...
    fn mastery_counts_and_avg_recall() {
        ...
        let m = compute_mastery(&tax, &cards);
    ...
    fn queue_ordering_by_ntr() {
        ...
        let ordered = order_by_ntr(&tax, &cards);
