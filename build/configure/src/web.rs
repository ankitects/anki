...
fn build_and_check_tslib(build: &mut Build) -> Result<()> {
    ...
    build.add_action(
        ...
        GenTypescriptProto {
            ...
            include_dirs: &["proto"],
            out_dir: "out/ts/generated",
            out_path_transform: |path| {
                path.replace("proto/", "ts/generated/")
                    .replace("proto\\", "ts/generated\\")
            },
    ...
    // copy source files from ts/generated
    build.add_action(
        "ts:generated:src",
        CopyFiles {
            inputs: inputs![glob!["ts/generated/*.ts"]],
            output_folder: "ts/generated",
        },
    )?;
