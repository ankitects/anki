        ...
        if let Some(item) = fsrs_item_for_memory_state(
            ...
            for entry in revlog {
                ...
                let memory_state: Option<FsrsMemoryState> = if revlog_index >= memory_states.len() {
                    // The removed revlog is in the end of the revlog, so we use
                    // the last memory state
                    ...
                    // The removed revlog is in the start of the revlog, so we
                    // don't have a memory state for it
                    None
                } else {
                    // The removed revlog is in the middle of the revlog, so we
                    // use the memory state for the previous
                    // revlog entry
