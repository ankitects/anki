...
fields, see the [editor](./editing) section.
...
reviewing; for that, see [templates](./templates/intro).
...
- **Editing Font** allows you to customize the font and size used when
  ...
  when typing the answer, see the [checking your answer](./templates/fields#checking-your-answer) section.)
...
<kbd>Ctrl</kbd>+<kbd>1-7</kbd> on Windows or <kbd>Cmd</kbd>+<kbd>1-7</kbd> on Mac) and
...
The editor is shown when [adding notes](./editing), [editing a note](./studying#editing-and-more) during reviews, or [browsing](./browsing).
...
about media, see the [media](/media) section.
...
correctly, see the section on [typing answers](./templates/fields#checking-your-answer).
...
### Styling answer highlights

To change the highlight over a revealed area on an IO card's answer side,
open the **Cards** screen for the Image Occlusion note type and select
**Styling**. In the existing `#image-occlusion-canvas` rule, change
`--highlight-shape-color` to a color with an alpha value. For example:

```css
#image-occlusion-canvas {
    --highlight-shape-color: #ff8e8e80;
}
```

The last two digits of this eight-digit hex color control opacity: `00` is
fully transparent, `80` is approximately 50% opaque, and `ff` is fully
opaque. The example adds a translucent pink highlight while keeping the
image underneath visible. The default, `#ff8e8e00`, has a transparent fill.
A six-digit color such as `#ff8e8e` is opaque and would cover the revealed
area again. This setting changes answer highlights, not the masks that hide
answers on the question side.
