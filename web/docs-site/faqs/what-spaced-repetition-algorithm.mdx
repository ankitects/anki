...
(see the [deck options](/manual/deck-options)
...
- Good  
  Moves the card to the [next step](/manual/deck-options?#learning-steps).
...
can set another value using the [Deck Options](/manual/deck-options?#starting-ease).
...
in a [previous FAQ.](/faqs/due-times-after-a-break)
