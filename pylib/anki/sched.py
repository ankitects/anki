...
class Scheduler:
    ...
    def answerCard(self, card, ease):
        ...
        if wasNewQ:
            ...
            # dynamic?
            if card.odid and card.type == QUEUE_TYPE_REV:
                if self._resched(card):
                    # reviews get their ivl boosted on first sight
                    card.ivl = self._dynIvlBoost(card)
                    card.odue = self.today + card.ivl
            self._updateStats(card, "new")
        if card.queue in (QUEUE_TYPE_LRN, QUEUE_TYPE_DAY_LRN):
...
order by due"""
    ...
    def countIdx(self, card):
        if card.queue == QUEUE_TYPE_DAY_LRN:
            return 1
        return card.queue
    ...
    def answerButtons(self, card):
        if card.odue:
            ...
            if card.type in (QUEUE_TYPE_NEW, QUEUE_TYPE_LRN) or len(conf["delays"]) > 1:
                return 3
            return 2
        elif card.queue == QUEUE_TYPE_REV:
            return 4
...
did in %s and queue = {QUEUE_TYPE_LRN} and due < ? limit %d)"""
        ...
        self.lrnCount += self.col.db.scalar(
            f"""
select count() from cards where did in %s and queue = {QUEUE_TYPE_DAY_LRN}
and due <= ? limit %d"""
            % (self._deckLimit(), self.reportLimit),
            self.today,
        )
...
did = ? and queue = {QUEUE_TYPE_DAY_LRN} and due <= ? limit ?""",
                did,
                self.today,
                self.queueLimit,
            )
    ...
    def _lrnConf(self, card):
        if card.type == QUEUE_TYPE_REV:
            return self._lapseConf(card)
        else:
            return self._newConf(card)

    def _rescheduleAsRev(self, card, conf, early):
        lapse = card.type == QUEUE_TYPE_REV
        if lapse:
            if self._resched(card):
...
where queue in ({QUEUE_TYPE_LRN},{QUEUE_TYPE_DAY_LRN}) and type = {QUEUE_TYPE_REV}
%s
"""
        ...
        self.forgetCards(
            self.col.db.list(f"select id from cards where queue in ({QUEUE_TYPE_LRN},{QUEUE_TYPE_DAY_LRN}) %s" % extra)
        )
...
(select left from cards where did = ? and queue = {QUEUE_TYPE_LRN} and due < ? limit ?)""",
        ...
        return cnt + self.col.db.scalar(
            f"""
select count() from
(select 1 from cards where did = ? and queue = {QUEUE_TYPE_DAY_LRN}
and due <= ? limit ?)""",
            did,
            self.today,
            self.reportLimit,
...
select id from cards where did in %s and queue = {QUEUE_TYPE_REV} and due <= ? limit ?)"""
    ...
    def emptyDyn(self, did, lim=None):
        ...
        self.col.db.execute(
            f"""
update cards set did = odid, queue = (case when type = {QUEUE_TYPE_LRN} then {QUEUE_TYPE_NEW}
else type end), type = (case when type = {QUEUE_TYPE_LRN} then {QUEUE_TYPE_NEW} else type end),
due = odue, odue = 0, odid = 0, usn = ? where %s"""
    ...
    def _moveToDyn(self, did, ids):
        ...
        queue = f"""
(case when type={QUEUE_TYPE_REV} and (case when odue then odue <= %d else due <= %d end)
...
To study outside of the normal schedule, click the Custom Study button below."""
    ...
    def nextIvl(self, card, ease):
        ...
        if card.queue in (QUEUE_TYPE_NEW, QUEUE_TYPE_LRN, QUEUE_TYPE_DAY_LRN):
