...
impl TryFrom<pb::Card> for Card {
    ...
    fn try_from(c: pb::Card) -> Result<Self, Self::Error> {
        let ctype = CardType::try_from(c.ctype as u8).invalid_input_context("invalid card type")?;
        let queue =
            CardQueue::try_from(c.queue as i8).invalid_input_context("invalid card queue")?;
        Ok(Card {
            id: CardId(c.id),
