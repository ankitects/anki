...
from testing.framework.java_codegen import JavaInvocationArgsGenerator, JavaFunctionArgTypeGenerator
...
class JavaArgGenTests(unittest.TestCase):
    ...
    def test_list_of_integer(self):
        ...
        for node in syntax_tree.nodes:
            args.append(argtype_generator.render(node, syntax_tree))
        self.assertEquals(len(args), 1)
        self.assertEquals('List<Integer>', args[0])
    ...
    def test_map(self):
        ...
        for node in syntax_tree.nodes:
            args.append(argtype_generator.render(node, syntax_tree))
        self.assertEquals(len(args), 1)
        self.assertEquals('Map<Integer, Integer>', args[0])
