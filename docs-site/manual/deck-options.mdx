...
For earlier versions, see [this FAQ page](/faqs/the-anki-2.1-scheduler).
...
- **Relative overdueness**: Shows cards that you're more likely to have forgotten first. This is generally recommended if
  ...
  reduce the chances of forgetting more cards.
    ...
    which shows the cards with the lowest current probability of recall first. Retrievability is calculated with each
    card's complete memory state and the current parameters of its home deck's preset.
...
For more info about burying cards, see [this section](./studying#siblings-and-burying) of the manual.
...
FSRS7 is the default model. Existing presets with no saved parameters automatically
adopt the FSRS7 defaults when the collection opens. Anki rebuilds their cards' memory
states from review history without changing the cards' current due dates or intervals.
This may take some time on the first open. Presets with saved parameters from an older
FSRS version keep using that model until you reset or optimize them.
...
The **New card intervals at graduation** table below this setting previews the
intervals produced by the current and selected desired retention. It covers each
answer on a new card and common two-answer paths through the learning steps. The
preview uses the unsaved learning steps, maximum interval, and FSRS parameters
currently shown in Deck Options.
...
_current_ desired retention, then check how it will change if you change desired
...
[this section](./deck-options#the-simulator).
...
Independent presets are optimized concurrently. The progress dialog shows overall
progress, active presets, review counts, elapsed time, an estimated remaining time,
and presets that were completed or skipped because they had no usable reviews.
...
- [Hard misuse](/manual/studying#answer-buttons). Hard
...
With FSRS7, configured (re)learning steps take precedence. Once no configured step
applies to an answer, FSRS can schedule another short-term interval, even after the
last step or on a review card. Leaving the steps empty gives FSRS control immediately.
Model intervals shorter than twelve hours use the learning/relearning queues; longer
intervals use whole days with the normal fuzz and maximum-interval rules. Crossing
the daily cutoff still uses Anki's interday-learning behavior.

Intraday learning/relearning answers do not consume the daily review limit. The
original review answer counts once; further intraday repetitions do not. Interday
learning/relearning still counts toward that limit. No separate same-day bypass
setting is needed. A short interval is not guaranteed: **Again** may still be a day
or longer. Explicit older FSRS parameter sets retain their existing step behavior.
...
Several frequently asked questions about FSRS have been answered in [its FAQ](/faqs/frequently-asked-questions-about-fsrs).
