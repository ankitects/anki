...
After that, TTS tags work without naming a voice. If your templates still
name Apple or Microsoft voices (common when a deck was built on macOS or
Windows), Anki uses the first available voice for that language. Restart
Anki after switching profiles if TTS stops; the sample add-on currently
registers its player only once per process.
