...
Each answer button shows the next time a card will be reviewed again if you select that button. To learn about the settings that control the next review intervals, see the topics [Learning Steps](deck-options.html#learning-steps), [Lapses](deck-options#lapses), [FSRS](deck-options.html#fsrs) and [Advanced](deck-options#advanced) in the Deck Options section.
