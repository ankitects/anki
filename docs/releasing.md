...
1. **`tools/prepare_release.py`** — Run first, on your own machine.
...
9. Review and merge the Flathub PR. When a stable release is published,
   `.github/workflows/sync-release.yml` opens a PR on
   [flathub/net.ankiweb.Anki](https://github.com/flathub/net.ankiweb.Anki)
   that moves the Flatpak to the new version. The PR is a draft if a patch
   does not apply or another step needs a manual fix. See
   [updating-anki.org](https://github.com/flathub/net.ankiweb.Anki/blob/master/updating-anki.org)
   for the manual steps. The job needs a `FLATHUB_TOKEN` secret that can push
   branches and open PRs on that repo.
...
1. Update the archive links in **`build/configure/src/audio.rs`** if you're updating the Windows pre-built binaries to new versions, then run `./tools/ninja audio_wheel` to test.
...
5. Once the PR is merged to main, run the **`.github/workflows/publish-audio-package.yml`** workflow to publish the package to PyPI:
    ```
    gh workflow run publish-audio-package.yml -f sign-macos=true -f publish-testpypi=true -f publish-pypi=true
    ```
