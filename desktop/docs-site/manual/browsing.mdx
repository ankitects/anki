...
<kbd>B</kbd>. It is comprised of three sections: the _sidebar_ on the left, the
_card/note table_ on the top right, and the _editing area_ on the bottom right. By
positioning the mouse between two sections, it is possible to click and drag to expand
one section and shrink the other.
...
<kbd>Cmd</kbd>+<kbd>Opt</kbd>+<kbd>T</kbd>. The switch also indicates if **C**ards or
**N**otes are currently shown.
...
<kbd>Ctrl</kbd>+<kbd>Shift</kbd> while clicking the Urdu deck in the sidebar to obtain
...
<kbd>F2</kbd> on Windows). Deletion even works for multiple items at once (see
