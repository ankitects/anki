...
sidebarTitle: "Filtered Decks"
description: "Filtered Decks"
---
...
times, and so on. See the [searching](./searching) section of the
...
When using FSRS, overdueness is calculated from the interval at which each card's
complete memory state reaches the desired retention in its home deck's current
preset.
