...
SonarCloud runs automatically on every push and pull request via CI. It aggregates
the coverage reports generated by the three stacks and runs static analysis across
Python, TypeScript, and Rust.

- **Dashboard**: https://sonarcloud.io/project/overview?id=ankitects_anki
- **Coverage**: uses the same LCOV/XML reports produced by `just test --coverage`
- **Analysis results**: SonarCloud posts a comment on each PR with quality gate status
