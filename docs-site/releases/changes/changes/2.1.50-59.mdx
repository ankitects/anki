...
[/manual/misc#debug-console](/manual/misc#debug-console)
...
- The sync server bundled with Anki has been updated, and now supports syncing
  ...
  [/manual/sync-server](/manual/sync-server).
...
You can try it out by using `mw.pm.set_new_import_export(True)` in the [debug console](/manual/misc#debug-console).
...
- A wheel is now provided for ARM64 Linux, and requires glibc 2.31 or greater. See
  [this page](/releases/betas#via-pypipip) for instructions on using it.
...
- [Windows](/manual/platform/windows/installing#qt5-vs-qt6)
- [Mac](/manual/platform/mac/installing#qt5-vs-qt6)
- [Linux](/manual/platform/linux/installing#qt5-vs-qt6)
...
- Intermediate deck limits now affect their children. Please see the [scheduler
  page](/faqs/the-2021-scheduler#daily-limits) for more
  info.
