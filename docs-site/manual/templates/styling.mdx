...
You can style [cloze deletions](/manual/editing#cloze-deletion) with two CSS
classes:

- `.cloze` applies to the active cloze for the current card: the hidden
  text's placeholder or hint on the question side, and the revealed text on
  the answer side.
- `.cloze-inactive` applies to other clozes whose text is shown on the
  current card, on both the question and answer sides.

For example, add the following to the note type's Styling section:
...
For a note containing `{{c1::Paris}} is the capital of {{c2::France}}`, the
first card shows `[...]` in bold and `France` in italics on the question
side. On the answer side, `Paris` is bold and `France` remains italic.
On the second card, `Paris` is italic on both sides, while the placeholder
for `France`, and then its revealed answer, are bold.
...
[webview changes](/addons/porting2.0#webview-changes)
...
2. In your computer’s file browser, go to your Anki application data folder (see [File Locations](../files#file-locations) for details), and then into your profile folder (e.g., "User 1").
...
javascript errors to the terminal, so you'll need to [view the console](/addons/console-output#console-output) to
see them. To debug issues with JavaScript, you can use Chrome's
[inspector](/addons/debugging#webviews).
