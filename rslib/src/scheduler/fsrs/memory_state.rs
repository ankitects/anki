...
impl Collection {
    ...
    pub(crate) fn update_memory_state(
        ...
        } in entries
        {
            ...
            // Store decay and desired retention in the card so that add-ons,
            // card info, stats and browser search/sorts don't need
            // to access the deck config. Unlike memory states,
            // scheduler doesn't use decay and dr stored in the card.
    ...
    fn update_memory_state_for_cards_with_items(
        ...
        // fsrs.memory_state_batch is O(nm) where n is the number of cards and m
        // is the max review count between all items. Therefore we want
        // to pass batches to fsrs.memory_state_batch where the review
        // count is relatively even.
    ...
    pub fn compute_memory_state(&mut self, card_id: CardId) -> Result<ComputeMemoryStateResponse> {
        ...
        // Get deck-specific desired retention if available, otherwise use
        // config default
    ...
    // Used for extra-ordinary circumstances where a memory state is needed but
    // is not availiable, e.g. the card has been moved to a different deck.
...
pub(crate) fn fsrs_item_for_memory_state(
    ...
    if let Some(mut output) = reviews_for_fsrs(entries, next_day_at, false, ignore_revlogs_before) {
        ...
        if output.revlogs_complete {
            ...
            // if the ease factor is less than 1.1, the revlog entry is
            // generated by FSRS
            if first_review.ease_factor <= 1.1 {
                starting_state.difficulty = (first_review.ease_factor - 0.1) * 9.0 + 1.0;
            }
            // remove the first review because it has been converted to the
            // starting state
        ...
        // no revlogs (new card or caused by ignore_revlogs_before or deleted
        // revlogs)
        Ok(None)
    }
}
...
mod tests {
    ...
    fn bypassed_learning_is_handled() -> Result<()> {
        // cards without any learning steps due to truncated history still have
        // memory state calculated
        ...
        // cards with a single review-type entry also get memory states from
        // revlog rather than card states
        let item = fsrs_item_for_memory_state(
            &fsrs,
            vec![RevlogEntry {
                ease_factor: 2500,
    ...
    fn zero_history_is_handled() -> Result<()> {
        ...
        // but memory state should still be inferred, by using the card's
        // current state
        let mut card = Card {
    ...
    mod update_memory_state {
        ...
        fn no_req_clears_fsrs_data() -> Result<()> {
            ...
            // Add a revlog entry so the card is found within
            // update_memory_state
