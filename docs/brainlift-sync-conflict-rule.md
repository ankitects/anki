...
moves its distinct pending local review to the first free millisecond inside
the original timestamp second, keeps the server payload at the shared ID, and
uploads the relocated review. The server uses the same bounded rule for a
distinct colliding upload. If all 1,000 millisecond IDs in that second are
occupied, sync fails rather than falsifying the review time. An identical
same-ID payload is a replay and remains a no-op.
...
- two different review payloads sharing one original revlog ID and surviving
  under one exact, time-preserving mapping on replicas with asymmetric
  unrelated high IDs;
