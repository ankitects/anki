...
impl ServerMediaDatabase {
    ...
    /// the list if the configured total bytes is exceeded.
    ///
    /// If any file entries were missing or deleted, we don't have any way in the current
    /// sync protocol to signal that they should be skipped, so we abort with a conflict.
    ...
    /// Delete provided file from media DB, leaving no record of deletion. It was probably
    /// missing due to an interrupted deletion, but removing the entry errs on the side of
    /// caution, ensuring the deletion doesn't propagate to other clients.
