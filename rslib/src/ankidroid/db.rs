...
impl Sizable for DbResult {
    fn estimate_size(&self) -> usize {
        // Performance: It might be best to take the first x rows and determine
        // the data types If we have floats or longs, they'll be a fixed
        // size (excluding nulls) and should speed up the calculation as
        // we'll only calculate a subset of the columns.
...
fn select_slice_of_size<'a>(
    ...
    rows.fold_while((0, init), |mut acc, x| {
        ...
        // If the accumulator is 0, but we're over the size: return a single
        // result so we don't loop forever. Theoretically, this
        // shouldn't happen as data should be reasonably sized
        if new_size > max_size && acc.0 > 0 {
            Done(acc)
        } else {
            // PERF: should be faster to return (size, numElements) then bulk
            // copy/slice
...
pub(crate) fn trim_and_cache_remaining(
    ...
    // PERF: Could speed this up by not creating the vector and just calculating
    // the count
...
mod tests {
    ...
    fn test_size_estimate() {
        ...
        let expected_size = (17 + 8 + 8) * 2; // 1 variable string, 1 long, 1
                                              // float
