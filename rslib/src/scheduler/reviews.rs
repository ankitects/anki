...
impl Card {
    ...
    fn set_due_date(
        ...
        let new_interval = if fsrs_enabled {
            // Don't set the interval if the card is new or if the card has an
            // interval of 0 (to prevent the interval changing if
            // set due date is used several times in a row on a new
            // card.)
