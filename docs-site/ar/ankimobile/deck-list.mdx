...
- **Add/Export** allows you to download shared decks, add/create an empty deck,
  import `.apkg` files from iTunes, and export your entire collection.
    - To import an `.apkg` or [properly formatted](/ar/manual/importing/text-files) plain-text `.csv`/`.tsv` file that you have saved on your device, tap on the file in your file manager and open it in AnkiMobile.
