...
It is calculated from the card's complete current memory state and the parameters
of its home deck's preset. In FSRS 7, this includes two internal stability traces
and difficulty; the displayed stability remains the interval at which
retrievability reaches 90%.
...
This table can be used to check how well [Anki's spaced repetition algorithm](/faqs/what-spaced-repetition-algorithm) is working for you.
