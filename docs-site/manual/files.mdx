...
<Note>
    The clearing out of unused tags is not synced to other devices with the rest of your
    collection, so you will need to run *Check Database* separately on each device.
</Note>
...
`~/Library/Application Support/Anki2` folder. The `~` stands for your home
folder. To open the Anki folder:

1. Open Finder and choose **Go → Go to Folder** from the menu bar, or press
   <kbd>Shift</kbd>+<kbd>Command</kbd>+<kbd>G</kbd>.
2. Paste `~/Library/Application Support/Anki2` and press <kbd>Return</kbd>.
3. Open the folder named after your Anki profile. For example, media for a
   profile named `User 1` is in `Anki2/User 1/collection.media`.

This works even though the Library folder is hidden by default. You can also
open Library from Finder's **Go** menu by holding down <kbd>Option</kbd> and
choosing **Library**, then opening **Application Support → Anki2**.
If you're on an older Anki version, your Anki files will be in your
`Documents/Anki` folder.
