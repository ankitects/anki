...
<Note>Replace 'anki' with 'launcher' for versions 25.07 to 25.09.4.</Note>
...
By default, the sync server opens its collection database with SQLite's exclusive
locking mode. If you need to read the collection database while the server is
running, you can set `SYNC_COLLECTION_DB_LOCKING_MODE=normal`.
