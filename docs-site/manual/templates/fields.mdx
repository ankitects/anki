...
On Windows, macOS, and iOS, Anki will use the OS’s built-in voices.

On Linux, Anki does not ship a TTS engine. Cards with `{{tts}}` tags will
show `no players found for TTSTag(...)` until you install a TTS add-on.
The sample [gTTS text to speech support](https://ankiweb.net/shared/info/391644525)
add-on is enough for many people:

1. **Tools → Add-ons → Get Add-ons…**
2. Paste `391644525` and click OK.
3. Restart Anki.

After that, `{{tts en_US:Front}}` works without naming a voice. If your
templates still name Apple or Microsoft voices (common when the deck was
built on macOS or Windows), Anki uses the first available voice for that
language. Restart Anki after switching profiles if TTS stops; the sample
add-on currently registers its player only once per process.

If you need a player that survives profile switches and can fall back to
`espeak-ng` when Google TTS is unreachable, see
[Linux TTS player](https://github.com/argrig666/anki-linux-tts-player).
...
already. See the [fields](../editing#customizing-fields) section if you’re not sure how
to do this.
